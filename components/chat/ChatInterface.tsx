...
export function ChatInterface({ projectId, chatId, initialMessages, initialInput, initialModel, initialImages, automationTaskId, phaseBoundaries }: ChatProps) {
    ...
    // Synchronous one-shot dispatch guard. Unlike `loading` (React state, which
    // updates asynchronously and can lag a render behind), this ref flips the
    // instant a submit starts — so a rapid double-click, an IME/Enter quirk, or
    // a StrictMode double-invoke can't fire two network calls for the same
    // message before the disabled state flushes. Exactly one run per submit.
    ...
    useEffect(() => {
        ...
        if (realtimeStatus === 'connected') {
            console.log('[Polling] Realtime is connected — polling disabled');
            return;
        }

        console.log(`[Polling] Realtime status="${realtimeStatus}" — starting polling fallback at ${new Date().toISOString()}`);
        let stopped = false;
        let pollCount = 0;
        ...
        const poll = async () => {
            ...
            if (isStreamingRef.current) {
                console.log(`[Polling] tick #${pollCount} — skipped (SSE stream active)`);
                return;
            }
            ...
            if (stopped) return;
            if (error) {
                console.error(`[Polling] tick #${pollCount} — DB error:`, error);
                return;
            }
            if (!data || data.length === 0) {
                console.log(`[Polling] tick #${pollCount} — no messages in DB yet`);
                return;
            }
            ...
            console.log(`[Polling] tick #${pollCount} — rebuilt ${rebuilt.length} UI messages, stillInflight=${stillInflight}`);
            if (!stillInflight) {
                console.log(`[Polling] tick #${pollCount} — turn resolved, clearing spinner`);
                setLoading(false);
                setThinkingText('');
            }
        };
    ...
    const handleSend = async (
        ...
        // Fires before any re-render flushes, so two near-simultaneous submits
        // (double-click, Enter twice, StrictMode remount) can't both reach the
        // network. The matching reset lives in the finally block below. Empty
        // submits return above WITHOUT touching this ref, so they never lock it.
        if (sendingRef.current) {
            console.warn('[handleSend] Ignored — a send is already in flight for this chat');
            return;
        }
        ...
        try {
            ...
            // ── 409 Conflict: a run is already in progress for this chat ──────
            // The backend enforces one run per chat_id. A 409 means our submit
            // was a duplicate and was correctly ignored — the ORIGINAL run is
            // still streaming. This is NOT a failure: do not retry, do not error,
            // and do not clear the loading state (the real run is still going).
            // Reconcile the UI from the DB so the canonical timeline of the
            // in-flight run replaces our optimistic duplicate bubbles, then keep
            // showing it. The Stop button stays available so the user can cancel
            // the current run before sending a new message.
            if (response.status === 409) {
                console.warn('[handleSend] 409 — a run is already in progress for this chat; keeping the original run.');
                await response.text().catch(() => undefined); // drain body
                setThinkingText('Already working on your last message…');

                // Pull the authoritative state from the DB and rebuild the
                // timeline (same transform as the polling fallback), so the
                // duplicate user/assistant bubbles we optimistically pushed are
                // replaced by the original run's real rows.
                try {
                    ...
                    if (data && data.length > 0) {
                        ...
                        if (rebuilt.length > 0) {
                            setMessages(rebuilt);
                            const stillInflight = rebuilt.some(
                                (m: any) => m.role === 'assistant' && m.isProcessing
                            );
                            // Keep loading true while the original run is live so
                            // the composer stays gated and the Stop button shows.
                            setLoading(stillInflight);
                            if (!stillInflight) setThinkingText('');
                        }
                    }
                } catch (reconcileErr) {
                    console.error('[handleSend] 409 reconcile failed:', reconcileErr);
                }
                // The finally block clears sendingRef/isStreamingRef so the
                // Realtime subscription (or polling) can keep updating the run.
                return;
            }
            // ─────────────────────────────────────────────────────────────────
            ...
            console.log(`[handleSend] finally — clearing isStreamingRef + sendingRef. realtimeStatus=${realtimeStatus}`);
            isStreamingRef.current = false;
            // Release the one-shot dispatch guard so the next genuine submit can
            // proceed. Loading state (not this ref) gates the UI while a run is
            // live, so clearing it here is safe even on the 409 path.
                    ...
                    <textarea
                        ...
                        onKeyDown={(e) => {
                            if (e.key === 'Enter' && !e.shiftKey) {
                                e.preventDefault();
                                // Ignore Enter while a run is in flight or being
                                // dispatched — the disabled textarea normally
                                // blocks this, but guard explicitly so an Enter
                                // that races the state flush can't double-submit.
                                if (loading || stopping || sendingRef.current) return;
                                handleSend();
                            }
                        }}
                        disabled={loading}
